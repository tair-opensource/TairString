...
int m_stringmatchlen(const char *pattern, int patternLen, const char *string, int stringLen, int nocase) {
    while (patternLen && stringLen) {
        switch (pattern[0]) {
            case '*':
                ...
                while (stringLen) {
                    if (m_stringmatchlen(pattern + 1, patternLen - 1, string, stringLen, nocase)) return 1; /* match */
                    string++;
                    stringLen--;
                }
                return 0; /* no match */
...
int m_string2l(const char *s, size_t slen, long *lval) {
    ...
    if (!m_string2ll(s, slen, &llval)) return 0;
...
int m_string2ld(const char *s, size_t slen, long double *dp) {
    ...
    if (isspace(buf[0]) || eptr[0] != '\0'
        || (errno == ERANGE && (value == HUGE_VAL || value == -HUGE_VAL || value == 0)) || errno == EINVAL
        || isnan(value))
        return 0;
